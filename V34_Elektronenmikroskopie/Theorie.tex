...
\section{Einleitung}
Die Transmissions-Elektronen-Mikroskopie (TEM) ermöglicht es, Bilder mit sehr großer Auflösung von atomaren Strukturen aufzunehmen. Man macht sich die Welleneigenschaft von stark beschleunigten Elektronen aufgrund von de Broglie zunutze . Somit erreicht man Wellenlängen im Bereich von $\lambda=4\pico\metre$.
...
Die Trajektorie eines Elektrons im TEM lässt sich in erster Näherung ebenso als geradlinigen Strahl beschreiben, weshalb die Betrachtung über die geometrische Optik zulässig ist.
...
\end{figure}
\subsection{Beugung}
Wir betrachten im Folgenden die Beugung der elektromagnetischen Welle an einem Objekt. Aufgrund des Teilchen-Wellen-Dualismuses gilt die Betrachtung auch für die im TEM beschleunigten Elektronen.
...
\end{align}
dabei sind $P_0$ und $P_1$ Ortspunkte der Bild- bzw. Beugungsebene, $U(P_0)$ die an dem Ort $P_0$ vorherrschende Feldstärke, $\vec{r_{01}}$ der Abstandsvektor zwischen $P_0$ und $P_1$ und $\theta$ der Winkel zwischen $\vec{r_{01}}$ und der optischen Achse. $\Sigma$ ist zudem die Fläche des beugenden Objekts und die Wellenzahl $k=\frac{2\pi}{\lambda}$. Eine ausführlichere Herleitung ist in \cite{lit:goodman96} zu finden.\\
...
\begin{align}
U(x,y)=\underbrace{\frac{\exp(\mathrm{i}kd)\cdot\exp\left(\frac{\mathrm{i}k}{2d}(x^2+y^2)\right)}{\mathrm{i}kd}}_{A(x,y,d)}\iint\limits_{-\infty}^{\infty}U(\xi,\eta)\exp\left(-\mathrm{i}\frac{2\pi}{\lambda d}(x\xi +y\eta)\right)\d\xi\;\d\eta,
\end{align}
dies ist die Fraunhofer Näherung. Die sogenannte \emph{Blendenfunktion} $U(\xi,\eta)$ ist definiert als
\begin{align}
U(\xi,\eta)=E(\xi,\eta)\cdot \tau(\xi,\eta),
\end{align}
...
Für zwei Funktionen $f(x)$ und $g(x)$ ist die Faltung $f*g$ definiert mit
\begin{align}
(f*g)(x)=\int\limits_{-\infty}^{\infty}f(t)g(x-t)\d t.
\end{align}
Für die Fouriertransformierte $\mathcal{F}$ der Faltung gilt dann
\begin{align}
\mathcal{F}[(f*g)(x)](\nu)=\mathcal{F}[f(x)](\nu)\cdot \mathcal{F}[g(x)](\nu).
\end{align}
Über Einsetzen der inversen Fouriertransformierten lässt sich ebenso zeigen, dass gilt:
\begin{align}
\mathcal{F}[f(x)](\nu)* \mathcal{F}[g(x)](\nu)=\mathcal{F}[(f\cdot g)(x)](\nu).
\end{align}
...
\dots &=\exp(-\mathrm{i}\pi\nu g(N-1))\cdot \frac{\sin(\pi\nu gN}{\sin(\pi\nu g)}.
\end{align}
In der Praxis haben die Gitterspalte eine endliche Breite $b$. Hier können wir das Faltungstheorem anwenden, und das Fraunhofer Beugungsbild des endlichen realen Gitters ist entsprechend
\begin{align}
\mathcal{F}[\tau(x)](\nu)=b\cdot\exp(-\mathrm{i}\pi\nu g(N-1))\cdot \frac{\sin(\pi\nu gN}{\sin(\pi\nu g)}\mathrm{sinc}(\pi b\nu).
\end{align}
...
Beispielsweise lässt sich das Beugungsbild eines rechteckigen Objekts als senkrechte Überlagerung von zwei Spalten auffassen. Oft wird eine Koordinatentransformation in Polarkoordinaten durchgeführt, beispielsweise für den Fall der Beugung an einer Lochblende mit Radius $\rho_0$. Dessen Transmissionsfunktion $\tau(\rho)$ ist definiert durch
\begin{align}
...
\begin{align}
\mathcal{F}[\tau(\rho)](r,d)=\dots =2\pi\rho_0^2\cdot\tfrac{J_1\left(\frac{kr\rho_0}{d}\right)}{\frac{kr\rho_0}{d}},
\end{align}
...
\begin{align}
\vec{g}_1=2\pi\tfrac{\vec{a}_2\times\vec{a}_3}{\left|\vec{a}_1\cdot (\vec{a}_2\times\vec{a}_3)\right|}\quad
\vec{g}_2=2\pi\tfrac{\vec{a}_3\times\vec{a}_1}{\left|\vec{a}_1\cdot (\vec{a}_2\times\vec{a}_3)\right|}\quad
\vec{g}_3=2\pi\tfrac{\vec{a}_1\times\vec{a}_2}{\left|\vec{a}_1\cdot (\vec{a}_2\times\vec{a}_3)\right|}.
\end{align}
...
\end{align}
wobei $a,~b,~c$ die Beträge der Basisvektoren in einem orthogonalen Koordinatensystem sind. Für ein kubisches Gitter mit $a=b=c$ vereinfacht sich der Ausdruck zu
\begin{align}
